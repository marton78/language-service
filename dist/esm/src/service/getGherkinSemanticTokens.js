...
export function getGherkinSemanticTokens(gherkinSource, expressions) {
    ...
    const tokenLines = walkGherkinDocument(gherkinDocument, [], {
        tag(tag, arr) {
        ...
        background(background, arr) {
            return makeLocationToken(background.location, background.keyword, SemanticTokenTypes.keyword, arr);
        },
        ...
        docString(docString, arr) {
            ...
            for (let lineNumber = docString.location.line; lineNumber <= maxLineNumber; lineNumber++) {
                ...
                if (spaceContent === null)
                    throw new Error(`No match for ${lines[lineNumber]}`);
...
function makeData(lines) {
    ...
    for (let lineNumber = 0; lineNumber < lines.length; lineNumber++) {
        ...
        for (const token of line) {
            const charDelta = lineNumber === lastLineNumber ? token.character - lastCharacter : token.character;
