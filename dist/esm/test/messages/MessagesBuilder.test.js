...
describe('MessagesBuilder', () => {
    it('does not fail on duplicate parameter types', () => __awaiter(void 0, void 0, void 0, function* () {
        const readStream = fs.createReadStream(`test/messages/dupe-parameter-types.ndjson`, 'utf-8');
        let receivedError;
        yield pipeline(readStream, new NdjsonToMessageStream(), new MessagesBuilderStream((err) => (receivedError = err)), new Writable({
            objectMode: true,
            write(_result, encoding, callback) {
                callback();
            },
        }));
        assert(receivedError);
    }));
    it('does not fail on cucumber expression syntax errors', () => __awaiter(void 0, void 0, void 0, function* () {
        const readStream = fs.createReadStream(`test/messages/syntax-error-cucumber-expression.ndjson`, 'utf-8');
        let receivedError;
        yield pipeline(readStream, new NdjsonToMessageStream(), new MessagesBuilderStream((err) => (receivedError = err)), new Writable({
            objectMode: true,
            write(_result, encoding, callback) {
    ...
    it('builds MessagesBuilder from a message stream with parameter types', () => __awaiter(void 0, void 0, void 0, function* () {
        const readStream = fs.createReadStream(`test/messages/messages.ndjson`, 'utf-8');
        let result;
        yield pipeline(readStream, new NdjsonToMessageStream(), new MessagesBuilderStream(), new Writable({
            objectMode: true,
            write(_result, encoding, callback) {
                result = _result;
                callback();
        ...
        assert.deepStrictEqual(result.expressions.map((e) => e.source), expectedExpressionSources);
    }));
});
