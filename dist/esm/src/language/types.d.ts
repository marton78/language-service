...
export type ParameterTypeName = 'int' | 'float' | 'word' | 'string' | 'double' | 'bigdecimal' | 'byte' | 'short' | 'long' | 'biginteger' | '';
/**
...
export type ParameterTypeMeta = Readonly<{
    name: string;
    regexp: string;
}>;
export declare const LanguageNames: readonly ["java", "tsx", "c_sharp", "php", "python", "ruby", "rust", "javascript", "go", "scala", "kotlin"];
...
export type ExpressionLink = Link & Readonly<{
    expression: Expression;
}>;
export type ParameterTypeLink = Link & Readonly<{
